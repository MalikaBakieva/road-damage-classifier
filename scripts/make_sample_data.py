...
def asphalt(rng: random.Random) -> Image.Image:
    ...
    if rng.random() < 0.5:  # lane marking
        ...
        draw.line(
            [(x, int(IMG_H * 0.3)), (x + rng.randint(-40, 40), IMG_H)],
            fill=(225, 225, 220),
...
def draw_damage(img: Image.Image, code: str, rng: random.Random) -> tuple[int, int, int, int]:
    ...
    if code == "D00":  # longitudinal crack — mostly vertical
        ...
        for _ in range(16):
            ...
            draw.line(
                [(x1, y1), (x1 + rng.randint(-40, 40), y1 + rng.randint(-35, 35))],
                fill=dark,
                width=2,
            )
        ...
        draw.ellipse(
            [cx - r + 6, cy - int(r * 0.7) + 5, cx + r - 6, cy + int(r * 0.7) - 5],
            outline=(70, 70, 70),
            width=3,
        )
...
def write_xml(
    path: Path, filename: str, boxes: list[tuple[str, tuple[int, int, int, int]]]
) -> None:
    ...
    for code, (xmin, ymin, xmax, ymax) in boxes:
        ...
        for tag, value in zip(
            ("xmin", "ymin", "xmax", "ymax"), (xmin, ymin, xmax, ymax), strict=True
        ):
